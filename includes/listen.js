...
(async function () {
  api.markAsReadAll((err) => {
	...
	//DEFINE DATLICH PATH
	const datlichPath = __dirname + '/../YumiModules/commands/cache/datlich.json';
		...
		for (el of temp) {
			...
			if ("ATTACHMENT" in el) {
				...
				for (a of el.ATTACHMENT) {
					...
					fs.writeFileSync(__dirname + `/../YumiModules/commands/cache/${a.fileName}`, Buffer.from(getAttachment, 'utf-8'));
					out.attachment.push(fs.createReadStream(__dirname + `/../YumiModules/commands/cache/${a.fileName}`));
				}
			}
			console.log(out);
			if ("BOX" in el) await api.setTitle(el["BOX"], el["TID"]);
			api.sendMessage(out, el["TID"], () => ("ATTACHMENT" in el) ? el.ATTACHMENT.forEach(a => fs.unlinkSync(__dirname + `/../YumiModules/commands/cache/${a.fileName}`)) : "");
		}
