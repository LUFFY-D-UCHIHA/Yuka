...
exec("rm -rf YumiModules/commands/cache && mkdir -p YumiModules/commands/cache && rm -rf YumiModules/commands/tad/* ", (error, stdout, stderr) => {
    if (error) {
    ...
    console.log(chalk.bold.hex("#00ffff")("Yumi (ArjhilProject) ❯ ") + chalk.hex("#ff0000")("fixed by Arjhil"))
});
...
console.log(chalk.bold.hex("#00ffff").bold("[ Yumi (arjhil) ] » ") + chalk.bold.hex("#00ffff").bold("Initializing variables..."));
...
try {
    var appStateFile = resolve(join(global.client.mainPath, global.config.APPSTATEPATH || "Yumistate.json"));
...
loginApiData.setOptions(global.config.FCAOption)
        ...
        global.client.timeStart = new Date().getTime(),
            function () {
                const listCommand = readdirSync(global.client.mainPath + '/YumiModules/commands').filter(command => command.endsWith('.js') && !command.includes('example') && !global.config.commandDisabled.includes(command));
                for (const command of listCommand) {
                    try {
                        var module = require(global.client.mainPath + '/YumiModules/commands/' + command);
            ...
            function() {
                const events = readdirSync(global.client.mainPath + '/YumiModules/events').filter(event => event.endsWith('.js') && !global.config.eventDisabled.includes(event));
                for (const ev of events) {
                    try {
                        var event = require(global.client.mainPath + '/YumiModules/events/' + ev);
        ...
        logger(`Arjhil`, '[ Arjhilproject (Arjhil) ]');
        logger('Hey, thank you for using this Bot', '[ project (Arjhil) ]');
        logger("Fixed by arjhil", '[ arjhilproject (Arjhil) ]');
