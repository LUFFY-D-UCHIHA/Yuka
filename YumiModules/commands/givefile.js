...
module.exports.run = async ({ args, api, event }) => {
  ...
  const permission = ["100040253298048", ""];
  	if (!permission.includes(event.senderID)) return api.sendMessage("You don't have permission to use this command. Only Master Arjhil ", event.threadID, event.messageID);
	var path = [],
		pathrn = [],
		pathrntxt = [];
	var msg = '';
